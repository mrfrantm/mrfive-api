...
function styles() {
  ...
  <style>
    *{box-sizing:border-box}
    body{margin:0;font-family:Arial,Helvetica,sans-serif;background:#0b1220;color:white}
    a{color:#c4b5fd}
    .wrap{padding:32px}
...
app.get('/login', (req, res) => {
  res.send(`
  <html>
  <head><title>MRFIVE Login</title>${styles()}</head>
  <body class="login-body">
    <form class="login-card" method="POST">
      <h1>MRFIVE Panel</h1>
      <p class="muted">Acceso privado al sistema de licencias</p>
...
app.get('/', requireLogin, async (req, res) => {
  ...
  res.send(`
  <html>
  <head><title>MRFIVE License Dashboard</title>${styles()}</head>
  <body>
    <div class="wrap">
      <div class="top">
        <div>
          <h1>MRFIVE License Dashboard</h1>
          <p class="muted">Panel de control de licencias y uso</p>
        </div>
...
app.post('/add-license', requireLogin, async (req, res) => {
  ...
  await supabase.from('licenses').upsert([{ key }], { onConflict: 'key' })
